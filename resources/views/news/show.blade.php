            ...
            <ul class="space-y-4">
                ...
                @foreach ($latestNews as $latest)
                    <li>
                        <!-- component -->
                        <div
                            class="bg-gray-100/50 dark:bg-gray-800 p-2 w-80 max-w-3xl sm:w-full sm:p-4 h-auto sm:h-48 rounded-2xl shadow-md flex flex-col sm:flex-row gap-5 select-none">

                            <a href="{{ route('news.show', $latest->slug) }}" style='background: url("{{ $latest->featured_image }}")'
                                class="h-48 sm:h-full sm:w-72 rounded-xl bg-gray-100 bg-center bg-cover"></a>
                            ...
                            <div class="flex flex-col justify-between p-1 sm:flex-1 space-y-2">
                                ...
                                <p class="text-gray-500 dark:text-white text-sm sm:text-base line-clamp-4">
                                    {{ $latest->body }}
                                </p>
                                ...
                                <div>
                                    <a href="{{ route('news.show', $latest->slug) }}"
                                        class="bg-firefly-700 w-auto px-2 text-white text-sm uppercase">Read More</a>
                                </div>
                            </div>
                        </div>
